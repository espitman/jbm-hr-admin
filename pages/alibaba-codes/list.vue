<template>
  <div class="container mx-auto p-0">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-xl font-bold text-purple-700">کدهای علی‌بابا</h1>
      <button
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="code in codes" :key="code.id">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ code.code }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ getTypeText(code.type) }}</td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ code.assign_to_user_id ? (code.assign_to_user?.first_name +' '+ code.assign_to_user?.last_name) : '-' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
import CreateCodeModal from '@/components/alibaba-codes/CreateCodeModal.vue'
...
const fetchCodes = async (page = 1) => {
  try {
    ...
    const res = await $api.get('/api/v1/admin/alibaba-codes', params)
